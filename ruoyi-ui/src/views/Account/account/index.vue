  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="身份" prop="identity">
        <el-select v-model="queryParams.identity" placeholder="请选择身份" clearable>
          <el-option
            v-for="dict in dict.type.identity_name"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
      ...
      <el-form-item label="报名组别" prop="scope">
        <el-select v-model="queryParams.scope" placeholder="请选择报名组别" clearable>
          <el-option
            v-for="dict in dict.type.scope_name"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
    ...
    <el-table v-loading="loading" :data="accountList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="身份" align="center" prop="identity">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.identity_name" :value="scope.row.identity"/>
        </template>
      </el-table-column>
      <el-table-column label="报名组别" align="center" prop="scope">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.scope_name" :value="scope.row.scope"/>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="报名组别" prop="scope">
          <el-select v-model="form.scope" placeholder="请选择报名组别">
            <el-option
              v-for="dict in dict.type.scope_name"
              :key="dict.value"
              :label="dict.label"
...
export default {
  name: "Account",
  dicts: ['scope_name', 'identity_name'],
  data() {
    return {
      // 遮罩层
      loading: true,
      ...
      rules: {
        ...
        scope: [
          { required: true, message: "报名组别不能为空", trigger: "change" }
        ],
        userName: [
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除账号管理编号为"' + ids + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('Account/account/export', {
        ...this.queryParams
